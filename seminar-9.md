...
**N-грамма** — последовательность из _n_ слов. Последовательность из двух последовательных элементов часто называют _биграмма_, последовательность из трёх элементов называется _триграмма_. Не менее четырёх и выше элементов обозначаются как _N-грамма_, N заменяется на количество последовательных элементов: 4-граммы, 5-граммы и т.д.
...
_Примечание._ Операцию деления нельзя использовать по тому же типу, что операцию умножения. Выражение "Время/100" означает, что система покажет, во сколько раз в текстах БД слово «время» встречается чаще (реже) чем цифра 100, а не уменьшит результат в сто раз. Это делает невозможной операцию вычисления средней встречаемости нескольких терминов.
